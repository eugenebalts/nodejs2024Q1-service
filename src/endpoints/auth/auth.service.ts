...
export class AuthService {
    ...
    async refresh(refreshTokenDto: RefreshTokenDto) {
        if (!("refreshToken" in refreshTokenDto) || typeof refreshTokenDto.refreshToken !== 'string') {
        ...
        try {
            ...
            const decodedToken = await this.jwtService.verifyAsync(refreshToken, { jwtid: 'refresh', secret: process.env.JWT_SECRET_REFRESH_KEY}).catch((err) => {throw new Error('Refresh token is invalid')});
    ...
    private async generateTokens(user: User): Promise<TokenPair> {
        const accessTokenPayload = { userId: user.id, login: user.login, secretKey: process.env.JWT_SECRET_KEY };
        const refreshTokenPayload = { login: user.login };

        const tokenPair: TokenPair = {
            accessToken: await this.jwtService.signAsync(accessTokenPayload, { secret: process.env.JWT_SECRET_KEY, expiresIn: process.env.TOKEN_EXPIRE_TIME }),
            refreshToken: await this.jwtService.signAsync(refreshTokenPayload, { jwtid: 'refresh', secret: process.env.JWT_SECRET_REFRESH_KEY, expiresIn: process.env.TOKEN_REFRESH_EXPIRE_TIME }),
        }
    ...
    private async isTokenValid(userId: string, refreshToken: string): Promise<boolean> {
        return (userId in this.tokenPairs && this.tokenPairs[userId].refreshToken === refreshToken);
    }
}